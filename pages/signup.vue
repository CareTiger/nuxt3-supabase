<template>
    <div>
        ...
        <form class="flex flex-col space-y-6 mt-6 w-64" @submit.prevent="signup">
...
const password = ref('');
const signup = async () => {
    const { data, error } = await supabaseAuthClient.auth.signUp({ email: email.value, password: password.value });
    if (error) {
